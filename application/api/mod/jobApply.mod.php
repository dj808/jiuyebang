<?php
	...
	class JobApplyMod extends CBaseMod {
		...
		public function getMyListInfo($id){
			...
			if(!count($data['job_tag_list']) || !$data['job_tag_list'])
				$data['job_tag_list'] = new stdClass();
			
			if($info['jobInfo']['money_lower'])
				$data['job_money'] = $info['jobInfo']['money_lower'].$info['jobInfo']['money_type_name'];
			else
				$data['job_money'] = $info['jobInfo']['money_upper'].$info['jobInfo']['money_type_name'];
