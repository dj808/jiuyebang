...
class BCRESTApi {
    ...
    static function registerApp($app_id, $app_secret, $master_secret = '', $test_secret = ''){
        if(empty($app_id)){
            throw new Exception(APIConfig::VALID_APP_ID);
        }
        self::$app_id = $app_id;
    ...
    static function get_common_params($data, $secret_type = '0'){
        ...
        if(empty($secret)){
            if(self::$mode){
                throw new Exception(APIConfig::NEED_PARAM. 'Test Secret, 请检查!');
            }else{
                throw new Exception(APIConfig::NEED_PARAM. 'APP/Master Secret, 请检查!');
            }
        }
